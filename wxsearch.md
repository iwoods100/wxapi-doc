...
sort: 排序，目前支持五种排序，分别为：0(默认排序), 1(按发布时间倒序，最新发布的排在前面), 2(按发布时间增序，最早发布的排在前面), 3(按文章入库时间倒序，最新入库的排在前面), 4(按文章入库时间增序，最早入库的排在前面)，默认为0
...
⚠️如果不想每次获取到重复的搜索文章，建议传sort=3，使返回的文章列表按入库时间进行排序即可，遇到之前返回过的文章就不需要再翻页了。
